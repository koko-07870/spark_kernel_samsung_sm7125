		...
		unsigned long len, unsigned long pgoff, unsigned long flags)
{
	...
	if (len > mmap_end - mmap_min_addr) {
		if (__ratelimit(&mmap_rs)) {
			printk(KERN_ERR "%s %d - (len > mmap_end - mmap_min_addr) len=0x%lx "
				"mmap_end=0x%lx mmap_min_addr=0x%lx pid=%d total_vm=0x%lx addr=0x%lx\n",
				__func__, __LINE__,
				len, mmap_end, mmap_min_addr, current->pid,
			  ...
			  unsigned long flags)
{
	...
	if (len > mmap_end - mmap_min_addr) {
		if (__ratelimit(&mmap_rs)) {
			printk(KERN_ERR "%s %d - (len > mmap_end - mmap_min_addr) len=%lx "
